"""Telegraph page creation for long responses."""
...
async def get_or_create_account(session: aiohttp.ClientSession) -> str:
    """Return a Telegraph access_token, creating an account if needed."""
    try:
    ...
    data = await _telegraph_call(
        session,
        "createAccount",
        short_name="OC-TG-Bot",
        author_name="OpenCode Bot",
    )
...
def _md_to_nodes(text: str) -> list:
    """Minimal markdown→Telegraph node conversion (paragraphs + code blocks)."""
    nodes: list = []
...
async def create_page(
    ...
    """Create a Telegraph page from markdown. Returns the page URL."""

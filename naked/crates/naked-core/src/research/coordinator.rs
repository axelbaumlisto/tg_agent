...
impl ResearchCoordinator {
    ...
    async fn verify_findings(&self, spec_id: &str) -> GatekeeperVerdict {
        let gk = &self.config.gatekeeper;
        let findings = self
            .store
        ...
        for f in &findings {
            ...
            if let Some(ref d) = f.listing_date
                && let Some(parsed) = super::tool::parse_listing_date(d)
            {
        ...
        if gk.detect_semantic_duplicates {
            let mut seen_signatures: std::collections::HashMap<String, &super::spec::Finding> =
                std::collections::HashMap::new();
            // Kept survivors with cached fuzzy fingerprints for the second pass.
            let mut kept: Vec<(&super::spec::Finding, FuzzyFingerprint)> = Vec::new();
            for f in &findings {
    ...
    async fn build_feedback_prompt(
        &self,
        spec_id: &str,
    ...
    async fn build_prompt(&self, spec: &ResearchSpec) -> Result<String> {
    ...
    async fn regenerate_report(&self, spec: &ResearchSpec) -> Result<()> {
    ...
    async fn regenerate_agent_brief(&self, spec: &ResearchSpec) -> Result<()> {
        let findings = self.store.list_findings(&spec.id, None).await?;
    ...
    #[allow(clippy::too_many_arguments)]
    async fn write_record_with_verification(
        &self,
        spec: &ResearchSpec,
        run_id: &str,
        new_findings: u32,

...
pub(crate) async fn research_cmd(args: &[String]) -> Result<()> {
    ...
    match sub {
        "ls" => cmd_ls(&agent).await,
        "show" | "delta" => cmd_show(&agent, &config, args, sub).await,
        "new" => cmd_new(&agent, args).await,
        "run" => cmd_run(&agent, args).await,
        "run-many" => cmd_run_many(&agent, args).await,
        "probe" => cmd_probe(&agent, args).await,
        "rm" => cmd_rm(&agent, args).await,
        "pause" => cmd_pause(&agent, args, true).await,
        "resume" => cmd_pause(&agent, args, false).await,
        "export" => cmd_export(&agent, args).await,
        "schedule" => cmd_schedule(args).await,
        "help" | "-h" | "--help" => {
            print_help();
            Ok(())
        }
...
async fn cmd_ls(agent: &Arc<AgentCore>) -> Result<()> {
...
async fn cmd_show(
    ...
    let fresh_only = sub == "delta" || args.iter().any(|a| a == "--fresh" || a == "--delta");
    let id = args
        .iter()
        .skip(1)
        .find(|a| !a.starts_with("--"))
        .ok_or_else(|| anyhow::anyhow!("usage: naked research show <id> [--fresh|--delta]"))?;
...
async fn cmd_new(agent: &Arc<AgentCore>, args: &[String]) -> Result<()> {
    let topic = args
        .iter()
        .skip(1)
        .find(|a| !a.starts_with("--"))
        .ok_or_else(|| anyhow::anyhow!("usage: naked research new <topic> [--source URL]..."))?
        .clone();
...
async fn cmd_run(agent: &Arc<AgentCore>, args: &[String]) -> Result<()> {
    let id = args
        .get(1)
    ...
    if matches!(
        report.stop_reason,
        naked_core::research::StopReason::Error | naked_core::research::StopReason::StreamClosed,
    ) {
        std::process::exit(1);
    }
    Ok(())
...
async fn cmd_run_many(agent: &Arc<AgentCore>, args: &[String]) -> Result<()> {
    ...
    for id in &ids {
        ...
        join_set.spawn(async move {
            ...
            if verify {
                run_many_verified_one(agent_c, id, verify_rounds, started_one).await
            } else {
                run_many_plain_one(agent_c, id).await
            }
        });
    }
...
async fn run_many_verified_one(
    agent: Arc<AgentCore>,
    id: String,
    verify_rounds: u32,
    started_one: std::time::Instant,
) -> Result<(String, String), (String, String)> {
    match agent.run_research_verified(&id, verify_rounds).await {
        Ok(vr) => Ok((
            id.clone(),
            format!(
...
async fn run_many_plain_one(
    agent: Arc<AgentCore>,
    id: String,
) -> Result<(String, String), (String, String)> {
    match agent.run_research(&id).await {
        Ok(report) => {
...
async fn cmd_probe(agent: &Arc<AgentCore>, args: &[String]) -> Result<()> {
    let urls = probe_urls(args)?;
    let topic = arg_values(args, "--topic")
        .first()
        .map(|s| s.to_string())
        .unwrap_or_else(default_probe_topic);
    ...
    let findings = store
        ...
        .await
        .unwrap_or_default();
    let stats = ProbeStats::from_findings(&findings);
    ...
    eprintln!(
        "  with real phone: {} ({} phone matches)",
        stats.findings_with_phone, stats.total_phones
    );
    eprintln!(
        "  captcha-prefix:  {}  (good fallback)",
        stats.captcha_excerpts
    );
    eprintln!(
        "  fake 'Liên hệ qua': {}  (must be 0)",
        stats.fake_lien_he_qua
    );
    if verbose {
        print_probe_findings_verbose(&findings);
    }
...
fn probe_urls(args: &[String]) -> Result<Vec<String>> {
...
struct ProbeStats {
    total_phones: u32,
    findings_with_phone: u32,
    captcha_excerpts: u32,
    fake_lien_he_qua: u32,
}

impl ProbeStats {
    fn from_findings(findings: &[naked_core::research::Finding]) -> Self {
        let mut stats = Self {
            total_phones: 0,
            findings_with_phone: 0,
            captcha_excerpts: 0,
            fake_lien_he_qua: 0,
        };
        for f in findings {
            ...
            if n > 0 {
                stats.findings_with_phone += 1;
                stats.total_phones += n;
            }
            if blob.contains("Contacts hidden behind site captcha") {
                stats.captcha_excerpts += 1;
            }
            if blob.to_lowercase().contains("liên hệ qua") {
                stats.fake_lien_he_qua += 1;
            }
        }
        stats
    }
...
fn count_vn_phones(text: &str) -> u32 {
    ...
    let is_word_boundary_left = |i: usize| {
        i == 0 || !bytes[i - 1].is_ascii_digit() && !matches!(bytes[i - 1], b'+' | b'.' | b'-')
    };
    let mut hits = 0u32;
    let mut i = 0;
    while i < bytes.len() {
        if bytes[i] == b'0' && is_word_boundary_left(i) {
            ...
            while j < bytes.len() {
                if bytes[j].is_ascii_digit() {
                    digits += 1;
                    j += 1;
                } else if is_sep(bytes[j]) && j + 1 < bytes.len() && bytes[j + 1].is_ascii_digit() {
                    j += 1;
                } else {
                    break;
                }
...
fn print_probe_findings_verbose(findings: &[naked_core::research::Finding]) {
    for f in findings {
...
async fn cmd_rm(agent: &Arc<AgentCore>, args: &[String]) -> Result<()> {
    let id = args
        .get(1)
...
async fn cmd_pause(agent: &Arc<AgentCore>, args: &[String], paused: bool) -> Result<()> {
    let verb = if paused { "pause" } else { "resume" };
    let id = args
        .get(1)
        .ok_or_else(|| anyhow::anyhow!("usage: naked research {verb} <id>"))?;
    agent.set_research_paused(id, paused).await?;
    if paused {
        eprintln!("paused {id}");
    } else {
        eprintln!("resumed {id}");
...
async fn cmd_export(agent: &Arc<AgentCore>, args: &[String]) -> Result<()> {
    let id = args
        .get(1)
...
async fn cmd_schedule(args: &[String]) -> Result<()> {
    let id = args
        .get(1)
        .ok_or_else(|| anyhow::anyhow!("usage: naked research schedule <id> [on|off|status]"))?;
...
fn print_help() {
    ...
    println!("           [-j N] [--verify [N]]    local job cap; respects naked.json permits");
    println!("  probe <url> [...]               fast iteration: ephemeral spec, single pass,");
    println!(

...
async fn main() -> Result<()> {
    ...
    if args.len() > 1 && args[1] == "research" {
        return research_cmd(&args[2..]).await;
    }
...
async fn research_cmd(args: &[String]) -> Result<()> {
    let sub = args.first().map(|s| s.as_str()).unwrap_or("help");
    let config = Config::load()?;
...
/// `naked agent <subcommand>` — generic agent-role primitives.
///
/// Built on the role + task + validator types in `naked_core::agent_role`,
/// `agent_run`, `agent_validator`. The CLI is intentionally thin —
/// every behaviour worth keeping lives in the library so `naked-tg`
/// (and any future webhook surface) can call the same code paths.
